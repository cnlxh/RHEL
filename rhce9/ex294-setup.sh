...
ansible-navigator:
  ...
  playbook-artifact:
    enable: false 
EOF
scp -o StrictHostKeyChecking=no /tmp/ansible-navigator.yml root@bastion:/home/greg/.ansible-navigator.yml &> /dev/null
ssh root@bastion 'chown greg:greg /home/greg/.ansible-navigator.yml -R' &> /dev/null
rm -rf /tmp/ansible-navigator.yml
}
function remove_repo_from_managed {
    ssh root@servera 'rm -rf /etc/yum.repos.d/*'
    ssh root@serverb 'rm -rf /etc/yum.repos.d/*'
    ssh root@serverc 'rm -rf /etc/yum.repos.d/*'
    ssh root@serverd 'rm -rf /etc/yum.repos.d/*'
    ssh root@workstation 'rm -rf /etc/yum.repos.d/*'
}
...
EOF
scp /tmp/ansible.repo root@bastion:/etc/yum.repos.d/ &> /dev/null
...
scp /tmp/ca.cert root@bastion:/etc/pki/tls/certs/utility.crt &> /dev/null
ssh root@bastion 'update-ca-trust'
rm -rf /tmp/ca.cert
...
virsh detach-disk serverd vdb --live --config &> /dev/null
ssh root@workstation 'systemctl set-default multi-user.target' &> /dev/null
ssh root@workstation 'reboot' &> /dev/null
